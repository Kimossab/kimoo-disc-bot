...
const renderDonut = (a: number, b: number) => {
  const cosA = cos(a);
  const sinA = sin(a);
  const cosB = cos(b);
  const sinB = sin(b);

  const output = new Array(SCREEN_WIDTH).fill(' ').map(() => new Array(SCREEN_HEIGHT).fill(' '));
  const zBuffer = new Array(SCREEN_WIDTH).fill(0).map(() => new Array(SCREEN_HEIGHT).fill(0));
